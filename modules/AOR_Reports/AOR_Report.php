...
class AOR_Report extends Basic
{
    ...
    public function build_report_html($offset = -1, $links = true, $group_value = '', $tableIdentifier = '', $extra = array())
    {
        ...
        //End
		//BBSMP-130 -- START
        $html.='<input id="sort_by" class="sort_by" type="hidden" value="'.$tableIdentifier.preg_replace("/[^A-Za-z0-9]/", '', $group_value).'"><table id="report_table_'.$tableIdentifier.preg_replace("/[^A-Za-z0-9]/", '', $group_value).'" cellpadding="0" cellspacing="0" width="100%" border="0" class="list view table-responsive aor_reports">';
		//BBSMP-130 -- END
        $sql = "SELECT id FROM aor_fields WHERE aor_report_id = '" . $this->id . "' AND deleted = 0 ORDER BY field_order ASC";
        $result = $this->db->query($sql);
        ...
        while ($row = $this->db->fetchByAssoc($result)) {
            ...
            if ($fields[$label]['display']) {
                ...
                $html .= "<div>";
                $html .= $field->label;
				$html .= "<span id=\"sort_table\" class=\"suitepicon suitepicon-action-sorting-none\" title=\"Sorted Table\"></span></div></th>";
			   //BBSMP-130 -- END
            }
            ++$i;
        }
        ...
        if ($offset >= 0) {
            ...
            if ($offset == 0) {
                ...
                //BBSMP-130 -- START
                $html .= "<button type='button' id='listViewStartButton_top' name='listViewStartButton' title='Start' class='list-view-pagination-button' onclick='changeReportPage(\"" . $this->id . "\",0,\"" . $group_value . "\",\"" . $tableIdentifier . "\",\"" . preg_replace("/[^A-Za-z0-9]/", '', $group_value) . "\")'>
                    <span class='suitepicon suitepicon-action-first'></span>
                </button>
                <button type='button' id='listViewPrevButton_top' name='listViewPrevButton' class='list-view-pagination-button' title='Previous' onclick='changeReportPage(\"" . $this->id . "\"," . $previous_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\",\"" . preg_replace("/[^A-Za-z0-9]/", '', $group_value) . "\")'>
                    <span class='suitepicon suitepicon-action-left'></span>
                </button>";
            }
            ...
            if ($next_offset < $total_rows) {
                $html .= "<button type='button' id='listViewNextButton_top' name='listViewNextButton' title='Next' class='list-view-pagination-button' onclick='changeReportPage(\"" . $this->id . "\"," . $next_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\",\"" . preg_replace("/[^A-Za-z0-9]/", '', $group_value) . "\")'>
                       <span class='suitepicon suitepicon-action-right'></span>
                    </button>
                     <button type='button' id='listViewEndButton_top' name='listViewEndButton' title='End' class='list-view-pagination-button' onclick='changeReportPage(\"" . $this->id . "\"," . $last_offset . ",\"" . $group_value . "\",\"" . $tableIdentifier . "\",\"" . preg_replace("/[^A-Za-z0-9]/", '', $group_value) . "\")'>
    ...
    public function build_report_query_where($query = array())
    {
        ...
        // BBSMP-277 --  start
		if( ($module->table_name == 'accounts') || ($module->table_name == 'contacts') ){
            if(!empty($query['where'])){
                $query['where'][] = ' AND '. $module->table_name.'.visibility = 1';
            }else{
                $query['where'][] =  $module->table_name.'.visibility = 1';
            }
        }
        if ($closure) {
            $query['where'][] = ')';
