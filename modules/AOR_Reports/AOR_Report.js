...
function appendHiddenFields(_form, ln, id) {
	...
	&& $("[name='aor_conditions_value\\[" + ln + "\\]").val()
	&& $("[name='aor_conditions_value\\[" + ln + "\\]").hasClass('DateTimeCombo')) {
		field_param = $("[name='aor_conditions_value\\[" + ln + "\\]']").val();
		field_param = updateTimeDateFields(field_param, ln);
	}else{
		field_param = $("#aor_conditions_value\\[" + ln + "\\]").val();
	}
	var fieldInput = '';
...
//BBSMP-130 -- START
function changeReportPage(record, offset, group_value, table_id, table_group_id) {
	//BBSMP-130 -- END
  ...
  $.get(query).done(
    function (data) {
      $('#report_table_' + table_id + table_group_id).replaceWith(data);
	  $( "#sort_by" ).remove();
    }
  );
  var Sort_By = $("#sort_by").val();
  $('#report_table_'+Sort_By+'').tablesorter({bFilter: false});
	//BBSMP-130 -- END  
}
