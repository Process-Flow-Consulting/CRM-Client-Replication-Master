...
class SavedReport extends SugarBean
{
	...
	function save_report($id, $owner_id, $name, $module,$report_type,$content,$is_published = 0,$team_id, $chart_type='none') {
        ...
        if (empty($name))
        {
            $name = translate('LBL_UNTITLED', 'AOR_Reports');
        }
		$json = getJSONobj();
...
//		if (isset($_REQUEST['view'])) {
			...
			$delete_line = "<a href=\"index.php?module=AOR_Reports&action=index&delete_report_id=".$this->id."\" class=\"listViewTdToolsS1\"> $delete_img ". $mod_strings['LBL_DELETE']."</a>";
			...
			$unpublish_line = "<a title='{$mod_strings['LBL_UN_PUBLISH']}' href=\"index.php?module=AOR_Reports&action=index&publish=no&publish_report_id=$this->id\" class=\"listViewTdToolsS1\">$unpublish_img</a>";
			$publish_line = "<a title='{$mod_strings['LBL_PUBLISH']}' href=\"index.php?module=AOR_Reports&action=index&publish=yes&publish_report_id=$this->id\" class=\"listViewTdToolsS1\">$publish_img</a>";
//		} else {
...
//		}
  ...
  function getACLAllowedModules() {
	...
	if (isset($_SESSION['reports_getACLAllowedModules'])) {
     ...
     require_once('modules/AOR_Reports/config.php');
