...
class AOR_ReportSchedule{

var $table_name='AOR_Report_schedules';
var $db;
...
function get_users_schedule($id=''){
		...
		while($row = $this->db->fetchByAssoc($results)){
			$return_array[$row['aor_report_id']] = $this->fromConvertReportScheduleDBRow($row);
		}
		return $return_array;
}

function get_report_schedule_for_user($aor_report_id, $user_id=''){
	if(empty($user_id)){
			global $current_user;
			$user_id = $current_user->id;
		}
	$query = "SELECT * FROM $this->table_name WHERE aor_report_id='$aor_report_id' AND user_id='$user_id' AND deleted=0";
	$results = $this->db->query($query);
	$row = $this->db->fetchByAssoc($results);
	return $this->fromConvertReportScheduleDBRow($row);
}

function get_report_schedule($aor_report_id){
	$query = "SELECT * FROM $this->table_name WHERE aor_report_id='$aor_report_id' AND deleted=0";
	$results = $this->db->query($query);
	$return_array = array();
	while($row = $this->db->fetchByAssoc($results)){
