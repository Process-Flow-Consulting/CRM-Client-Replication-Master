...
// in modules/AOS_Quotes/language/<lang>.lang.php
global $layouts;
$layouts = array(
	'Standard'=>'modules/AOS_Quotes/layouts/Standard.php',
    'Invoice'=>'modules/AOS_Quotes/layouts/Invoice.php'
);
...
function print_layout($layout) {
    ...
    if(!isset($layouts[$layout])) {
		$GLOBALS['log']->fatal("quote layout is not registered in modules/AOS_Quotes/Layouts.php");
		sugar_die ("quote layout is not registered in modules/AOS_Quotes/Layouts.php");
...
if(file_exists('modules/AOS_Quotes/Layouts.override.php')) {
	include_once('modules/AOS_Quotes/Layouts.override.php');
}
if(file_exists('custom/modules/AOS_Quotes/Layouts.php')) {
	include_once('custom/modules/AOS_Quotes/Layouts.php');
}

if(isset($_REQUEST['email_action']) && $_REQUEST['email_action']=="EmailLayout") {
	//check to make sure the layout is set
	if (isset($_REQUEST['layout'])) {
		include_once('modules/AOS_Quotes/EmailPDF.php');
	...
	if($email_id=="") {
		...
		header("Location: index.php?action=Compose&module=Emails&parent_type=AOS_Quotes&return_module=AOS_Quotes&return_action=DetailView&return_id=".$_REQUEST['record']."&recordId=$email_id"."&parent_id=".$_REQUEST['record']);
	}
//end if email layout
} else {
	if (isset($_REQUEST['layout'])) {
