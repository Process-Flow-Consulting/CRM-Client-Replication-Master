...
class AOS_QuotesSugarpdfStandard extends AOS_QuotesSugarpdfAOS_Quotes{
    ...
    */
    ...
    function preDisplay(){
        ...
        //@date 07-oct-2013
		//BBSMP-204 -- start
		$oppQuery = "SELECT op.id,op.lead_address,op.lead_state,op.lead_county,op.lead_city,op.lead_zip_code FROM aos_quotes AS q_op";
		$oppQuery .= " INNER JOIN opportunities AS op1 ON op1.id = q_op.opportunity_id";
		$oppQuery .= " INNER JOIN opportunities AS op ON op.id = COALESCE(op1.parent_opportunity_id,op1.id) WHERE q_op.id = '".$this->bean->id."'";
		//BBSMP-204 -- end
    ...
    function display(){
		...
		// BBSMP-204 & BBSMP-219 -- Start
        $addressBS[][$mod_strings['LBL_PDF_BILLING_ADDRESS']]  = $this->bean->billing_contact;
		if(!empty($this->bean->billing_contact_id)){
			$contact = new Contact();
			$contact->retrieve($this->bean->billing_contact_id);
			 $addressBS[][$mod_strings['LBL_PDF_BILLING_ADDRESS']]  = $contact->email1;
		}
        $addressBS[][$mod_strings['LBL_PDF_BILLING_ADDRESS']]  = $this->bean->billing_account;
