...
*}
{{include file=custom/modules/Quotes/tpls/detail_view_header.tpl}}
...
{{else}}
	...
	{{if !empty($label) && !is_int($label) && $label != 'DEFAULT' && !$useTabs}}
		{{if $label == 'LBL_DETAILVIEW_PANEL1'}}			
			<!-- hirak : date: 12-10-2012 -->
			{if $proposal_delivery_mrthod neq 'M'}
				{if $verify_proposal_sent}
				<h4>{sugar_translate label='{{$label}}' module='{{$module}}'}				
					<span id="show_verify_message_sent" style="font-size: 12px; margin: 0px; padding: 0 0 0 30px; color: #FF0000;">{$MOD.LBL_SHOW_VERIFICATION_SENT_MESSAGE}</span>
				</h4>
				{else}
				<h4>{sugar_translate label='{{$label}}' module='{{$module}}'}				
					<span id="show_verify_message" style="font-size: 12px; margin: 0px; padding: 0 0 0 30px; color: #FF0000; display:none;">{$MOD.LBL_SHOW_VERIFY_MESSAGE}</span>
				</h4>
				{/if}
			{/if}									
		{{else}}
