...
<th align="left" colspan="8">
	{{if $label == 'detailpanel_1'}}
		<h4>{sugar_translate label='{{$label}}' module='{{$module}}'}		
			<span id="show_verify_message" style="font-size: 12px; margin: 0px; padding: 0 0 0 30px; color: #FF0000; display: none;">Note: Proposals will not be sent until the Proposal has been verified.</span>
		</h4>										
	{{else}}
	...
	{{/if}}
		...
		<td valign="top" id='{{$colData.field.name}}_label' width='{{$def.templateMeta.widths[$smarty.foreach.colIteration.index].label}}%' scope="col">
			{{if isset($colData.field.customLabel)}}
			   <label for="{{$fields[$colData.field.name].name}}"><input type="checkbox" />{{$colData.field.customLabel}}</label>
            ...
            {{if isset($colData.field.popupHelp) || isset($fields[$colData.field.name]) && isset($fields[$colData.field.name].popupHelp) }}
              ...
              {{/if}}
              {capture name="overlibStuff" assiis_form_updatedgn="overlibStuff"}{overlib_includes}{/capture}
