...
function updateProposals ()
{
    ...
    $sqlGetProposals = "SELECT * FROM oss_proposalqueue " .
             " WHERE deleted = 0 AND instance_folder_name = '" .
             $arAdminData->settings['instance_account_name'] .
             "' AND ((proposal_schedule_status = 'scheduled' AND process_stat = '2') " .
             "   OR (proposal_schedule_status = 'delivered' AND process_stat = '4' AND DATEDIFF(NOW(),date_schedule) <=5 )) " .
    ...
    while ($arResult = $cdb->fetch_assoc($queryGetProposals)) {
        ...
        if ($completed == true) {
            ...
            if ($bChangeSalesStage) {
                $opportunity_sql = " SELECT opportunity_id FROM quotes_opportunities
				WHERE quotes_opportunities.quote_id = '" .
                         $arResult['proposal_id'] . "'
