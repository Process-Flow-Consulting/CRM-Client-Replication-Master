...
class AOS_QuotesViewDetail extends ViewDetail 
{
    ...
    /**
 	...
 	{
		global $beanFiles, $app_strings, $db, $app_list_strings, $current_user;
		...
		$taxRateName = '';
		if(!empty($taxRateId)){
    		$sqlTaxRates = "SELECT id, name, value FROM aos_taxrates WHERE deleted = 0 AND status = 'Active' AND id = '".$taxRateId."' ";
		...
		$line_items = $product->get_full_list('indexs',$where_li);
		$inclusions = $product->get_full_list('indexs',$where_inc);
		$exclusions = $product->get_full_list('indexs',$where_exc);
		$alternates = $product->get_full_list('indexs',$where_alt);
		...
		//Get Project Opportunity of Quotes
		$opp = new Opportunity();
		...
		$account = new Account();
		$account->retrieve($this->bean->billing_account_id);
		$account_proview_link = proview_url(array('url'=>$account->proview_url));
		$account_name = '<a href="index.php?module=Accounts&action=DetailView&record='.$this->bean->billing_account_id.'">'.$this->bean->billing_account.'</a>';
		...
		$percentSign = str_replace("In ",'',translate('LBL_IN_PERCENTAGE', 'AOS_ProductTemplates'));
		...
		if(PDF_CLASS == 'TCPDF') {
		    $this->dv->defs['templateMeta']['form']['links'] = array('PDF Format: <select name="sugarpdf" id="sugarpdf">{$LAYOUT_OPTIONS}</select></form>');
			// Bug 41079 Check User Email Client Type
			$userPref = '';
            global $sugar_config;
			$defaultPref = $current_user->getPreference("email_link_type");
			if($userPref != '') {
				$client = $userPref;
			} else {
				$client = $defaultPref;
		...
		*/	
			...
			$pdfViewButton .= '<input id="print_as_pdf_button" title="Print as PDF" class="button" type="submit" name="button" value="Print as PDF"></form>';
		...
		$this->th->deleteTemplate($this->module, 'DetailView');
 		...
 		parent::display();
		echo "<script>$('#billing_address_street').parent().css('height','50px');</script>";
