...
$sugar_smarty->assign('SUBTHEMES', $this->bean->getSubThemes());
$sugar_smarty->assign('SUBTHEME', $this->bean->getSubTheme());
...
$sugar_smarty->assign('PHONE_HOME', formatPhoneNo($focus->phone_home));
$sugar_smarty->assign('PHONE_MOBILE', formatPhoneNo($focus->phone_mobile));
$sugar_smarty->assign('PHONE_WORK', formatPhoneNo($focus->phone_work));
$sugar_smarty->assign('PHONE_OTHER', formatPhoneNo($focus->phone_other));
$sugar_smarty->assign('PHONE_FAX', formatPhoneNo($focus->phone_fax));
...
$sugar_smarty->assign('COMPANY_NAME', $focus->company_name);
$sugar_smarty->assign('AR_STATES', $GLOBALS['app_list_strings']['state_dom']);
...
$sugar_smarty->assign('CUR_ID',$current_user->id);
...
function formatPhoneNo($rawField){
	
	$rawField = clean_text($rawField);
	if(strlen($rawField) < 10){
		$new_field = preg_replace("/([0-9]{3})([0-9]{4})/", "$1-$2", $rawField);
	}elseif(strlen($rawField) >= 10){
		$new_field = preg_replace("/([0-9]{3})([0-9]{3})([0-9]{4})/", "$1-$2-$3", $rawField);
	}else{
		$new_field =  $rawField; 
	}
	return $new_field;
