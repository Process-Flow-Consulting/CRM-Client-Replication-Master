...
class UnifiedSearchAdvanced {
    ...
    var $searchFormClass = 'cusSearchForm';
	...
	function getDropDownDiv($tpl = 'custom/modules/Home/UnifiedSearchAdvanced.tpl') {
	...
	function search() {
		 ...
		 * Added By :Ashutosh
		 * Date : 19 March 2013
		 * Purpose : to separate the Project and Client Opportunities 
		 */
		if(isset($modules_to_search['Opportunities'])){
		    
		    unset($modules_to_search['Opportunities']);
		    $modules_to_search['ParentOpportunity'] = 'Opportunity';
		    $modules_to_search['Opportunities']= 'Opportunity';
		
		    $unified_search_modules['ParentOpportunity'] = $unified_search_modules['Opportunities'];
		    //$unified_search_modules['ParentOpportunity']['fields']['parent_opportunity_id']= 
		...
		if(!empty($this->query_string)) {
			 ...
			 * Added By Ashutosh
			 * Date : 15 March 2013
			 * purpose : to restrict the pagination in client and project opp
			 */	
			if((isset($_REQUEST['isParnetOpp']) || isset($_REQUEST['isChildOpp']) )&& $moduleName == 'Opportunities')
		    {
		      
		        unset($_POST['isChildOpp'],$_GET['isChildOpp'],$_REQUEST['isChildOpp']);
						    
				$_REQUEST['Home2_OPPORTUNITY_offset']= $iOffsetForChild;
				$_POST['Home2_OPPORTUNITY_offset']= $iOffsetForChild;
				$_GET['Home2_OPPORTUNITY_offset']= $iOffsetForChild;
				
		    }
		    if(isset($_REQUEST['isChildOpp']) && $moduleName == 'ParentOpportunity')
		    {
		       
		        $iOffsetForChild = $_REQUEST['Home2_OPPORTUNITY_offset'];
		        $iOffsetForChild = $_POST['Home2_OPPORTUNITY_offset'];
		        $iOffsetForChild = $_GET['Home2_OPPORTUNITY_offset'];
		        
		        unset($_POST['isParnetOpp'],$_GET['isParnetOpp'],$_REQUEST['isParnetOpp']);
				$_REQUEST['Home2_OPPORTUNITY_offset']= 0;
				$_POST['Home2_OPPORTUNITY_offset']= 0;
				$_GET['Home2_OPPORTUNITY_offset']= 0;
				
				    
		    }
		    
			   // echo '<br/>'. $moduleName = ($moduleName == 'ParentOpportunity')?'Opportunities':$moduleName;
                 ...
                 * Added By : Ashutosh
                 * Date : 29 Jan
                 * Purpose : to change the seed to bbProjectLeads
                 */                
                if($beanName == 'Lead'){
                ...
                if($beanName == 'Call'){
                    require_once 'custom/modules/Calls/customCall.php';
                    $seed = new customCall();
                }
                if($beanName == 'Meeting'){
                    require_once 'custom/modules/Meetings/customMeeting.php';
                    $seed = new customMeeting();
                }
                /**
                 * Added By : Ashutosh
                 * Date : 19 March 2013
                 * Purpose : to apply parent and client opportunity conditions
                 */
                if($moduleName == 'ParentOpportunity'){
                   	require_once 'custom/modules/Opportunities/OpportunitySummary.php';
                   	$_REQUEST['searchFormTab'] =1;    
		            $seed = new OpportunitySummary();
                }
                ...
                //hirak - account and contacts visibility check
                if($seed instanceof Account){
                	$where .= ' AND accounts.visibility = 1 ';
                }
                
                if($seed instanceof Contact){
                	$where .= ' AND contacts.visibility = 1 ';
                }
                //hirak - account and contacts visibility check
                /**
                 * Added By : Ashutosh
                 ...
                 */
                if($moduleName == 'ParentOpportunity'){
                    $where .= ' AND parent_opportunity_id IS NULL ';
                    $GLOBALS['app_list_strings']['moduleList'][$seed->module_dir] = 'Project Opportunities';
                }
                
                if($moduleName == 'Opportunities'){
                    $where .= ' AND parent_opportunity_id IS NOT NULL ';
                    $GLOBALS['app_list_strings']['moduleList'][$seed->module_dir]= 'Client Opportunities';
                }
                /* END OF client and parent opp conditions */
                 ...
                 *  Added By : Ashutosh 
                 * Date : 19 March 2013 
                 * Purose : to add extra param for parent opp
                */
                 if($moduleName == 'ParentOpportunity'){
					//echo '<pre>';print_r($lv->data['pageData']['urls']);echo '</pre>';
					foreach($lv->data['pageData']['urls'] as  $iKey =>$stUrlString){
						$lv->data['pageData']['urls'][$iKey] = str_replace('module=Home','module=Home&isParnetOpp=1',$stUrlString );
						if(strstr( $lv->data['pageData']['urls'][$iKey],'isChildOpp')){
						    $lv->data['pageData']['urls'][$iKey] = str_replace('&isChildOpp=1','',$lv->data['pageData']['urls'][$iKey] );
						}
						
					}
					//echo '<pre> PARA';print_r($lv->data['pageData']['urls']);echo '</pre>';
				}elseif($moduleName == 'Opportunities'){
					
					foreach($lv->data['pageData']['urls'] as  $iKey =>$stUrlString){
						$lv->data['pageData']['urls'][$iKey] = str_replace('module=Home','module=Home&isChildOpp=1',$stUrlString );
						if(strstr( $lv->data['pageData']['urls'][$iKey],'isParnetOpp')){
						    $lv->data['pageData']['urls'][$iKey] = str_replace('&isParnetOpp=1','',$lv->data['pageData']['urls'][$iKey] );
						}
					}
					//echo '<pre> CHCH';print_r($lv->data['pageData']['urls']);echo '</pre>';
				} 
