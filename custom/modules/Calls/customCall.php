...
class customCall extends Call{
	...
	function create_new_list_query($order_by, $where, $filter=array(), $params=array(), $show_deleted = 0, $join_type='', $return_array = false, $parentbean=null, $singleSelect = false) {
		...
		if(isset($_REQUEST['date_from_advanced']) && !empty($_REQUEST['date_from_advanced'])){
            ...
            $stLeadSQL['where'] .= " AND calls.date_start >='".$timedate->to_db($date_from)."' ";
       }
       if(isset($_REQUEST['date_to_advanced']) && !empty($_REQUEST['date_to_advanced'])){
            ...
            $stLeadSQL['where'] .= " AND calls.date_start <='".$gmt_date_to['end']."'  ";
       }
	...
	function fill_in_additional_list_fields() {
		...
		if(isset($call->contact_id) && !empty($call->contact_id)){
				
			$this->parent_id = $call->contact_id;
