 ...
 ********************************************************************************/
  ...
  *********************************************************************************/
  ...
  $email->email2init();
  $ie = new CustomInboundEmail(); //separeate client and project opportunity -- hirak
  $ie->email = $email;
  $json = getJSONobj();
  ...
  switch($_REQUEST['emailUIAction']) {
    ...
    case "composeEmail":
        ...
        if(isset($_REQUEST['sugarEmail']) && $_REQUEST['sugarEmail'] == 'true' && isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
            ...
            if( !empty($ret['parent_type']) &&  ($ret['parent_type'] == 'Opportunities')
            && !empty($ret['parent_id']) ){
            	...
            	$class = $beanList[$ret['parent_type']];
            	require_once($beanFiles[$class]);
            	 
            	$bean = new $class();
            	$bean->retrieve($ret['parent_id']);
            	 
            	if($bean->module_dir == 'Opportunities'){
            		if(empty($bean->parent_opportunity_id))
            			$ret['parent_type'] = 'ProjectOpportunities';
            		else
            			$ret['parent_type'] = 'ClientOpportunities';
            	}
            }
    ...
    case "sendEmail":
        ...
        //separeate client and project opportunity -- hirak
       	if($_REQUEST['parent_type'] == 'ProjectOpportunities' || $_REQUEST['parent_type'] == 'ClientOpportunities'){
       		$_REQUEST['parent_type'] = 'Opportunities';
       	}
       	/**
       	 * One to many email from opportunities
       	 * Modified by : Ashutosh
       	 * Date : 13 Nov 2013
       	 */
       	//send email to all cient opportunities 
        if(isset($_REQUEST['oppOneToMany'])){
            $arRequestData = $_REQUEST;
     
            //get all email to addresses and send to individual email
            foreach ($arRequestData['to_clients'] as $iKey => $stEmailIds){
                //set to email address
                $_REQUEST['sendTo'] = $stEmailIds;
                $_REQUEST['addressTO0'] = $stEmailIds;
                //set related client Opportunity id
                $_REQUEST['parent_id'] = $arRequestData['related_opp'][$iKey];
                //send email
                $email->email2Send($_REQUEST);  
               
            }
            
            //loop through the to email addresses and send seperate email
            $ret = array(
    ...
    case 'saveQuickCreate':
         ...
         * Added By : Ashutosh
         * Date : 24 Dec 2013
         * Purpose : If Opportunity modules is geting saved
         * then save client opportunity
         */
        if ($_REQUEST['qcmodule'] == 'Opportunities' && !isset($_REQUEST['parent_opportunity_id'])) {
            
                        
            $oss_timedate = new OssTimeDate();            
            $controller->bean->date_closed = $oss_timedate->convertDateForDB($_REQUEST['date_closed'], $_REQUEST['bid_due_timezone']);    
            
            $obOpportunity = clone $controller->bean;
            unset($controller->bean->account_name);
            unset($controller->bean->account_id);
            unset($controller->bean->contact_name);
            unset($controller->bean->contact_id);
        }
         ...
         */
        if ($_REQUEST['qcmodule'] == 'Opportunities' && !isset($_REQUEST['parent_opportunity_id'])) {
            // set and save client Opportunity
            $obClientOpp = new Opportunity();
                      
            
            $obClientOpp->name = $obOpportunity->name;
            $obClientOpp->date_closed = $controller->bean->date_closed;
            $obClientOpp->bid_due_timezone = $obOpportunity->bid_due_timezone; 
            $obClientOpp->sales_stage = $obOpportunity->sales_stage;
            $obClientOpp->amount = $obOpportunity->amount;
            $obClientOpp->amount_usdoller = $obOpportunity->amount_usdoller;
            $obClientOpp->currency_id = $obOpportunity->currency_id;
            $obClientOpp->account_name = $obOpportunity->account_name;
            $obClientOpp->account_id = $obOpportunity->account_id;
            $obClientOpp->account_id = $obOpportunity->account_id;
            $obClientOpp->contact_name = $obOpportunity->contact_name;
            $obClientOpp->contact_id = $obOpportunity->contact_id;	
            $obClientOpp->project_lead_id = $obOpportunity->project_lead_id;	            		           
			
            $obClientOpp->team_id = $_REQUEST['copTeamSet_collection_' . $_REQUEST['primary_copTeamSet_collection']];
            $obClientOpp->assigned_user_id = $_REQUEST['cop_assigned_user_id'];
            $obClientOpp->parent_opportunity_id = $controller->bean->id;
