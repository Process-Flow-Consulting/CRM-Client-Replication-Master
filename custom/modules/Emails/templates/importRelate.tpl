...
function parent_typechangeQS() {
    ...
    sqs_objects[parentFieldName]['parent_opportunity_only'] = '0';
    QSFieldsArray[parentFieldName].sqs.parent_opportunity_only = '0';
    if(new_module == 'ProjectOpportunities'){
    	sqs_objects[parentFieldName]['parent_opportunity_only'] = '1';
    	QSFieldsArray[parentFieldName].sqs.parent_opportunity_only = '1';
    }
    if(new_module == 'ProjectOpportunities' || new_module == 'ClientOpportunities'){
        new_module = 'Opportunities';
    }
...
//separeate client and project opportunity -- hirak
function open_related_popup(){

	if(document["{/literal}{$formName}{literal}"].parent_type.value != ""){

		var module = document["{/literal}{$formName}{literal}"].parent_type.value;
		if( document["{/literal}{$formName}{literal}"].parent_type.value == "ProjectOpportunities" 
			|| document["{/literal}{$formName}{literal}"].parent_type.value == "ClientOpportunities"){
			var module = "Opportunities";
		}
		
		var params = '';
		if(document["{/literal}{$formName}{literal}"].parent_type.value == "ProjectOpportunities" ){
			params = "&parent_opportunity_only=true";
		}
		open_popup(
			module,
			600,
			...
			false,
			{
				...
				"form_name":"{/literal}{$formName}{literal}",
...
<td nowrap="nowrap" width="37%">
<input name="assigned_user_name" class="sqsEnabled sqsNoAutofill" tabindex="2" id="assigned_user_name" size="" value="{$userName}" type="text">
...
<input type="text" value="" tabindex="2" name="parent_name" id="parent_name" class="sqsEnabled sqsNoAutofill" autocomplete="OFF"/>
<input type="button"  onclick="open_related_popup();" value="{$APP.LBL_SELECT_BUTTON_LABEL}"  title="{$APP.LBL_SELECT_BUTTON_TITLE}" class="button" tabindex="2" name="button" id="change_parent"/>
</slot>
</td></tr></table>
</td>
</tr>
