...
//for quickcompose in Opportunity
elseif(isset($data['forQuickCreateOppMails'])){
    $ret = generateOneToManyComposeDataPackage($data);
}
...
function generateOneToManyComposeDataPackage($data, $forFullCompose = TRUE)
{
    // we will need the following:
    global  $db;
     //$email = new Email();
     //$namePlusEmail = $email->getNamePlusEmailAddressesForCompose($_REQUEST['action_module'], (explode(",", $_REQUEST['uid'])));
    $arSelectedIds = explode(",", $_REQUEST['uid']);
    unset($arSelectedIds[count($arSelectedIds)]);
    $stInIds = "'".implode("','",$arSelectedIds)."'"; 
    /*$stSelectRelatedEmailsSql = 'SELECT  accounts.id account_id,opportunities.id,accounts.name, eabr.primary_address, ea.email_address
    FROM `opportunities` 
    LEFT JOIN accounts_opportunities acop on acop.opportunity_id = opportunities.id and acop.deleted=0
    LEFT JOIN accounts on acop.account_id = accounts.id and accounts.deleted =0
    ...
    WHERE opportunities.id IN  ('.$stInIds .') AND opportunities.deleted = 0 AND eabr.primary_address=1 ORDER BY eabr.primary_address DESC';
    */
    $stSelectRelatedEmailsSql = 'SELECT  contacts.id contact_id,opportunities.id
            , CONCAT( COALESCE(salutation,"")," " , COALESCE( first_name,  " " ) ,  " ", COALESCE( last_name,  " " ) ) name
            , ea.opt_out opt_out, ea.invalid_email invalid_email, eabr.primary_address, COALESCE( ea.email_address,"none") email_address
    FROM `opportunities`    
    LEFT JOIN contacts on contacts.id = opportunities.contact_id and contacts.deleted =0
    LEFT JOIN email_addr_bean_rel eabr ON (contacts.id = eabr.bean_id and eabr.deleted=0) 
    LEFT JOIN email_addresses ea ON (eabr.email_address_id = ea.id) 
    WHERE opportunities.id IN  ('.$stInIds .') AND opportunities.deleted = 0 
		 AND COALESCE(eabr.primary_address,1) =1   AND contacts.id IS NOT NULL 
            ORDER BY eabr.primary_address DESC';
    
    
    $rsRelatedAddress = $db->query($stSelectRelatedEmailsSql);
    $emailOptOut = array();
    $invalidEmail = array();
    while($arFetchedRow = $db->fetchByAssoc($rsRelatedAddress)){
        $returndata[$arFetchedRow['id']] = from_html($arFetchedRow['name']) . " <".from_html($arFetchedRow['email_address']).">";
        $emailOptOut[$arFetchedRow['id']] = $arFetchedRow['opt_out'];
        $invalidEmail[$arFetchedRow['id']] = $arFetchedRow['invalid_email'];
    }
    $namePlusEmail = join(',',array_values($returndata));
    $ret = array (
            'to_email_addrs' => $namePlusEmail,
            'email_link_map' => $returndata, 
