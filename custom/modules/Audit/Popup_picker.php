...
require_once('custom/modules/Audit/Audit.php');
...
EOHTML;
		...
		if($_REQUEST['module_name'] == 'Leads'){
			$params[] = translate('LBL_CHANGE_LOG', 'Audit')." - ".$focus->project_title;			
		}elseif($_REQUEST['module_name'] == 'Opportunities'){
			$params[] = translate('LBL_CHANGE_LOG', 'Audit')." - ".$focus->name;
		}else{
		...
		if(!($_REQUEST['module_name'] != 'Leads' || $_REQUEST['module_name'] != 'Opportunities')){
			if($field_count > 0)
			{
				$index = 0;
		...
		foreach($audit_list as $audit)
		{
			...
			if(trim($audit['before_value_string']) == '' && trim($audit['after_value_string']) == '')
			{
			...
			//BBSMP-341 -- Start
			if(!empty($before_value) && $_REQUEST['module_name'] == 'Leads' && $focus->scope_of_work)
			$before_value = html_entity_decode($before_value, ENT_COMPAT, 'UTF-8');
		
			if(!empty($after_value) && $_REQUEST['module_name'] == 'Leads' && $focus->scope_of_work)
			$after_value = html_entity_decode($after_value, ENT_COMPAT, 'UTF-8');
			//BBSMP-341 -- End
