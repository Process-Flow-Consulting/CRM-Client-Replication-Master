...
class UserViewHelper {
    ...
    protected function setupButtonsAndTabs() {
        ...
        if (!$this->bean->is_group){
            ...
            /**
             * Added by Ashutosh to reset pref. and reset home page buttons
             */
    ...
    protected function setupThemeTab() {
        ...
        if(isset($user_theme)) {
        	/*
        	 * removed the classic theme option
        	 */
            $this->ss->assign("THEMES", str_replace("<OPTION value='Sugar5'>Classic</OPTION>", '', get_select_options_with_id(SugarThemeRegistry::availableThemes(), $user_theme)));
        } else {
        	/*
        	 * removed the classic theme option
        	*/
            $this->ss->assign("THEMES", str_replace("<OPTION value='Sugar5'>Classic</OPTION>", '', get_select_options_with_id(SugarThemeRegistry::availableThemes(), $GLOBALS['sugar_config']['default_theme'])));
        }
...
// Build a list of themes that support group modules
    ...
    protected function setupEmailSettings() {
        ...
        $this->ss->assign("NEW_EMAIL",  '<span id="email_span">' . getEmailAddressWidgetCustom($this->bean, "email1", $this->bean->email1, $this->viewType) .' </span>');
