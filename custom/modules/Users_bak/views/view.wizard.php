...
class ViewWizard extends SugarView
{
	...
	public function display()
    {
		...
		$this->ss->assign('PHONE_FAX', $current_user->phone_fax);
		$this->ss->assign('COMPANY_NAME', $current_user->company_name);
        ...
        if($current_user->address_state )
	    {
	    	$stSelecteState = $current_user->address_state;
	        require_once 'custom/include/OssTimeDate.php';
	        $obOssTimeDate = new OssTimeDate();
	        //get users timezone
	        $stSelecteState = $GLOBALS['app_list_strings']['state_dom'][$stSelecteState];
	        ...
	        $userTZ = $current_user->user_preferences['timezone'] = $stTimeZone;
...
eoq;
        ...
        $this->ss->assign('AR_ALL_STATES',$GLOBALS['app_list_strings']['state_dom']);
