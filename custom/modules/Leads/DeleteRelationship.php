...
if($_REQUEST['linked_field']=='leads_online_plans' && $_REQUEST['module']=='Leads'){
	
	require_once 'custom/include/common_functions.php';
	require_once 'modules/oss_OnlinePlans/oss_OnlinePlans.php';
	global $db,$current_user;
	//mark this online plan deleted
	$obOnlinPlan = new oss_OnlinePlans();
	$obOnlinPlan->mark_deleted($linked_id);
	...
	$insertSQL = "INSERT INTO leads_audit (`id`,`parent_id`,`date_created`,`created_by`,`field_name`,`data_type`,`before_value_string`,`after_value_string`) VALUES (UUID(),'" . $lead_id . "',UTC_TIMESTAMP(),'" . $current_user->id . "','Online Plan Modification','datetimecombo','" . $result ['before_date'] . "',UTC_TIMESTAMP());";
	$db->query ( $insertSQL );
