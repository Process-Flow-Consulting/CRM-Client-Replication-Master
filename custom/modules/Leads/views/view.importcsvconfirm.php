...
class LeadsViewImportcsvconfirm extends SugarView
{
    const SAMPLE_ROW_SIZE = 3;
    protected $errorScript = "";
    protected $currentFormID = 'importcsvconfirm';
    protected $previousAction = 'importcsvstep1';
    protected $nextAction = 'importcsvstep2';
    ...
    function display(){
        global $mod_strings, $app_list_strings, $app_strings, $current_user, $sugar_config, $locale;
        ...
        {
            $this->_showImportError('Import Error: No File Uploaded.', 'Leads', 'importcsvstep1', false, null , true);
            return;
        }
        ...
        if( isset($_FILES['leadfile']['size']) && ($_FILES['leadfile']['size'] == 0) && !empty($uploadLeadFileName) ){
            ...
            $this->_showImportError('Import Error: Empty Project Leads CSV.', 'Leads', 'importcsvstep1', false, null , true);
            return;
        }
        ...
        if( isset($_FILES['bidderfile']['size']) && ($_FILES['bidderfile']['size'] == 0) && !empty($uploadBidderFileName) ){
            //this file is empty, throw error message
            $this->_showImportError('Import Error: Empty Bidders CSV.', 'Leads', 'importcsvstep1', false, null , true);
            return;
        }
        ...
        && !empty($uploadLeadFileName) ) {
            ...
            $this->_showImportError($error_msgs, 'Leads', 'importcsvstep1', false, null , true);
            $mimeTypeOk = false;
        }
        ...
        if(empty($uploadLeadFileName)){
            $this->nextAction = 'importcsvstep3';
        }
        ...
        //tpl file
        $this->ss->display('custom/modules/Leads/tpls/importcsvconfirm.tpl');
    }
...
{
    document.getElementById('importcsvconfirm').action.value = '{$this->previousAction}';
    return true;
}

document.getElementById('gonext').onclick = function()
{
    document.getElementById('importcsvconfirm').action.value = '{$this->nextAction}';
    return true;
}
EOQ;
        return $javascript;
