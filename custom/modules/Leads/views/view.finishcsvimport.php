...
class ViewFinishcsvimport extends SugarView 
{	
    function ViewFinishcsvimport(){
    	parent::SugarView();
    }
 	...
 	public function display()
    {
        ...
        $this->ss->assign("errorrecordsFile",ImportCacheFiles::convertFileNameToUrl(ImportCacheFiles::getErrorRecordsFileName()));
        ...
        $stCountSQL = "SELECT bean_type ,count(bean_type) total,case bean_type when 'Lead' then 0 when 'Account' then 1 else 2 end  ord FROM  users_last_import WHERE bean_type = 'Lead' OR bean_type = 'Account' group by bean_type order by ord";
        ...
        $this->ss->display('custom/modules/Leads/tpls/finish_csv_import.tpl');
...
document.getElementById('importmore').onclick = function(){
    ...
    document.getElementById('importlast').action.value = 'importcsvbidders';
    return true;
}
