...
<form enctype="multipart/form-data" real_id="importcsvstep3" id="importcsvstep3" name="importcsvstep3" method="POST" action="index.php">

{foreach from=$smarty.request key=k item=v}
    {if $k neq 'current_step' && $k neq 'action'  &&  $k neq 'previous_action' &&  $k neq 'module' 
           && $K neq 'lead_column_count' && $k neq 'bidder_column_count' && strncasecmp($k, "bidder_colnum_", 14) != 0 }
        {if is_array($v)}
...
<input type="hidden" name="lead_column_count" value="{$smarty.request.lead_column_count}">
<input type="hidden" name="bidder_column_count" value="{$COLUMNCOUNT}">
...
<input type="hidden" name="lead_firstrow" value="{$smarty.request.lead_firstrow}">
<input type="hidden" name="bidder_firstrow" value="{$FIRSTROW}">
...
<tr>
    ...
    {/if}
    <td valign="top" align="left" id="bidder_row_{$smarty.foreach.rows.index}_col_0">
        <select class='fixedwidth' name="bidder_colnum_{$smarty.foreach.rows.index}">
