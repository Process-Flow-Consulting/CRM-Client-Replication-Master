...
SE.composeLayout = {
    ...
    */
    ...
    addDocumentField : function(idx) {
        ...
        documentCup.innerHTML = "<input type='hidden' name='document" + idx + index + "' id='document" + idx + index + "' />" +
                ...
                "<button class='button firstChild' type='button' name='documentSelect" + idx + index + "' id='documentSelect" + idx + index + "'" +
                    "onclick='SE.composeLayout.selectDocument(\"" + index + "\",\""+ idx +"\");' value='" + app_strings.LBL_EMAIL_SELECT + "'>" +
    ...
    selectDocument : function(target,idx) {
        if(document.getElementById('data_parent_type'+idx).value == 'Quotes'){
            var parentId = document.getElementById('data_parent_id'+idx).value;        
            URL="index.php?module=Emails&action=PopupDocuments&to_pdf=true&target=" + target+'&parent_id='+parentId;
        }else{
