...
class SugarWidgetSubPanelTopSelectProposalDocumentButton extends SugarWidgetSubPanelTopButton
{
	//button_properties is a collection of properties associated with the widget_class definition. layoutmanager
	function SugarWidgetSubPanelTopSelectProposalDocumentButton($button_properties=array())
	{
	...
	function display(&$widget_data)
	{
		...
		$popup_request_data = array(
			'call_back_function' => 'set_selected_doc',//'set_return_and_save_background',
		...
		return ' <input type="button" name="' . $this->getWidgetId() . '_select_button" id="' . $this->getWidgetId() . '_select_button" class="button"' . "\n"
				. ' title="' . $this->title . '"'
			. ' value="' . $this->value . "\"\n"
			. " onclick='open_popup(\"$this->module_name\",600,400,\"$initial_filter&proposal_docs=1\",true,true,$json_encoded_php_array,\"$popup_mode\",$create);' />\n";
	}
}
?>