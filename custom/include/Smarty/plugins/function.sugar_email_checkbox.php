...
/**
 * Smarty {smarty_function_sugar_email_checkbox} function plugin
 *
 * Type:     function
 * Name:     sugar_email_checkbox
 * Purpose:  generate a checkbox based on passsed values
 * 
 * @author Ashutosh
 * @param array
 * @param Smarty
 */
function smarty_function_sugar_email_checkbox($params, &$smarty)
{
	global $db;
	if (!isset($params['module']) && !isset($params['module_id'])){	
		return '';
	}
	$stContactId = $params['module_id'];
    ...
    WHERE  COALESCE(eabr.primary_address, 1) = 1  AND contacts.id = '{$stContactId}'
    ORDER BY eabr.primary_address DESC";
	
	$rsResult = $db->query($stSQL);
