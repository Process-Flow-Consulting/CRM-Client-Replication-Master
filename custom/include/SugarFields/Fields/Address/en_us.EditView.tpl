 ...
 * Copy Primary Address Field to Alternate Address Field with hidden State field if module is Contacts
*/
*}
{if $module eq 'Contacts'}
{assign var="jsFile" value="custom/modules/Contacts/addressFieldJs/addressField.js"}
{else}
{assign var="jsFile" value="include/SugarFields/Fields/Address/SugarFieldAddress.js"}
{/if}
<script type="text/javascript" src='{sugar_getjspath file=$jsFile}'></script>
...
{{assign var="state_options" value=$displayParams.key|cat:'_state_options'}}
...
<td>
<div id="{{$state}}_div">
{html_options name="{{$state}}" id="{{$state}}" options=$fields.{{$state}}.options selected=$fields.{{$state}}.value}
</select>
</div>
{if $module eq 'Contacts'}
<input type="hidden" name="{{$state}}" id="{{$state}}_free" value="{$fields.{{$state}}.value}" />
{/if}
</td>
</tr>
<tr>
