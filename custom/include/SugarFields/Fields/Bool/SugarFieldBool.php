...
class SugarFieldBool extends SugarFieldBase {
	...
	function getSearchViewSmarty($parentFieldArray, $vardef, $displayParams, $tabindex) {
		...
		//If there was a type override to specifically render it as a boolean, show the EditView checkbox
                ...
                if( preg_match("/(favorites|current_user|open|fav_bidders)_only.*/", $vardef['name']))
		{
			return $this->fetch($this->findTemplate('EditView'));
                        
		}   else if(preg_match("/(sector_served|labor_affiliation|certification).*/", $vardef['name'])){
                    
                        return $this->fetch($this->findTemplate('ClientSearch'));
